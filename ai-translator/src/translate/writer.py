...
from schema import Book, TextContent, TableContent
...
class Writer:
    ...
    def save_translated_book(self, book: Book, output_format: str) -> str:
        ...
        if output_format.lower() == 'pdf':
            ...
            logger.error(f"不支持文件类型: {output_format}")
            raise ValueError(f"不支持文件类型: {output_format}")

        logger.info(f"已翻译文件保存至: {output_file_path}")

        return output_file_path

    def _save_translated_book_pdf(self, book: Book):

        output_file_path = book.file_path.replace('.pdf', f'_translated.pdf')

        logger.info(f"开始导出: {output_file_path}")
