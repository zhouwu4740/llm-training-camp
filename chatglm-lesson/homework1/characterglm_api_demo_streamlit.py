...
# updated for 作业一，设置图像风格列表
IMAGE_STYLES = ["二次元", "卡通", "极简", "波普", "超现实"]
...
with st.container():
    ...
    with col1:
        ...
        st.selectbox(label="图片风格", key="image_style", on_change=lambda: st.session_state["meta"].update(
            image_style=st.session_state["image_style"]), options=IMAGE_STYLES, index=0)
